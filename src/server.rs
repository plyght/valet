...
async fn call(
    ...
    if let Err(e) = authorize(&state, &headers) {
        audit_end(&request_id, &origin, token_present, &req.tool, "deny", &e.code(), started.elapsed().as_millis() as u64, 0, None);
        return into_response(e).into_response();
    }
    ...
    let Some(tool) = state.registry.get(&req.tool) else {
        audit_end(&request_id, &origin, token_present, &req.tool, "deny", &AppError::NotFound.code(), started.elapsed().as_millis() as u64, 0, None);
    ...
    if req.stream {
        match tool.call_stream(req.params).await {
            Ok(body) => {
                audit_end(&request_id, &origin, token_present, &req.tool, "allow", "OK", started.elapsed().as_millis() as u64, 0, Some(true));
                (
                    StatusCode::OK,
            ...
            Err(e) => {
                audit_end(&request_id, &origin, token_present, &req.tool, "error", &e.code(), started.elapsed().as_millis() as u64, 0, Some(true));
        ...
        match tool.call(req.params).await {
            Ok(result) => {
                ...
                audit_end(&request_id, &origin, token_present, &req.tool, "allow", "OK", started.elapsed().as_millis() as u64, bytes_out, Some(false));
            ...
            Err(e) => {
                ...
                audit_end(&request_id, &origin, token_present, &req.tool, "error", &e.code(), started.elapsed().as_millis() as u64, bytes_out, Some(false));
...
fn audit_end(request_id: &str, origin: &str, token_present: bool, tool: &str, decision: &str, code: &str, duration_ms: u64, bytes_out: u64, streaming: Option<bool>) {
    tracing::info!(
