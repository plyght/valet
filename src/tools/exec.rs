...
use tokio::{io::{AsyncReadExt, BufReader}, process::Command, time::{timeout, Duration}};
...
impl ExecTool {
    pub fn new(cfg: &Config) -> anyhow::Result<Self> {
        ...
        Ok(Self { allowed: resolved, pass_env: cfg.exec.pass_env.clone(), timeout_s: cfg.limits.exec_timeout_s, max_stdout_kb: cfg.limits.max_stdout_kb })
    }
}
...
fn resolve_cmds(cmds: &[String]) -> anyhow::Result<HashSet<PathBuf>> {
    ...
    for c in cmds {
        let path = if c.contains('/') { PathBuf::from(c) } else { which::which(c)? };
...
impl Tool for ExecTool {
    ...
    async fn call(&self, params: serde_json::Value) -> Result<serde_json::Value, AppError> {
        let cmd = params.get("cmd").and_then(|v| v.as_str()).ok_or_else(|| AppError::ToolError("missing cmd".into()))?;
        ...
        let timeout_s = params.get("timeout_s").and_then(|v| v.as_u64()).map(|t| t.min(self.timeout_s)).unwrap_or(self.timeout_s);

        let full = if cmd.contains('/') { dunce::canonicalize(cmd).map_err(|_| AppError::ExecDenied)? } else { which::which(cmd).map_err(|_| AppError::ExecDenied)? };
        let full = dunce::canonicalize(full).map_err(|_| AppError::ExecDenied)?;
        if !self.allowed.contains(&full) { return Err(AppError::ExecDenied); }
        ...
        command.env_clear();
        for k in &self.pass_env { if let Ok(v) = std::env::var(k) { command.env(k, v); } }

        let start = Instant::now();
        let mut child = match command.spawn() { Ok(c) => c, Err(_) => return Err(AppError::Internal("failed to spawn".into())) };
        ...
        if timed_out { let _ = child.kill().await; }
        let status = match timeout(to, child.wait()).await { Ok(Ok(s)) => s, _ => return Err(AppError::ExecTimeout) };
        ...
        Ok(json!({"exit_code": exit_code, "stdout_b64": stdout_b64, "stderr_b64": stderr_b64, "duration_ms": duration_ms, "truncated": truncated, "timed_out": timed_out}))
    }

    async fn call_stream(&self, params: serde_json::Value) -> Result<crate::server::StreamBody, AppError> {
        ...
        tokio::spawn(async move {
            ...
            command.stderr(Stdio::piped());
            command.env_clear();
            for k in &pass_env { if let Ok(v) = std::env::var(k) { command.env(k, v); } }
            let mut child = match command.spawn() { Ok(c) => c, Err(_) => { let _ = tx.send(line(json!({"event":"error","error":{"code":"Internal","message":"failed to spawn"}}))).await; return; } };
            let mut stdout = child.stdout.take().unwrap();
            let mut stderr = child.stderr.take().unwrap();
            ...
            match status {
                Err(_) => { let _ = tx.send(line(json!({"event":"error","error":{"code":"ExecTimeout","message":"timeout"}}))).await; }
                Ok(_) => { let _ = tx.send(line(json!({"event":"end","result": {"duration_ms": start.elapsed().as_millis() as u64 }}))).await; }
            }
        });

        let body = Body::from_stream(ReceiverStream::new(rx).map(|s| Ok::<_, std::io::Error>(s)));
        Ok(body)
    }
}
