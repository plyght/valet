...
pub struct FsWriteTool { root: PathBuf }
impl FsWriteTool { pub fn new(cfg: &Config) -> anyhow::Result<Self> { Ok(Self { root: cfg.root.root_dir.clone() }) } }
...
impl Tool for FsWriteTool {
    ...
    async fn call(&self, params: serde_json::Value) -> Result<serde_json::Value, AppError> {
        ...
        let content_b64 = params.get("content_b64").and_then(|v| v.as_str()).ok_or_else(|| AppError::ToolError("missing content_b64".into()))?;
        ...
        if let Some(parent) = full.parent() { fs::create_dir_all(parent).map_err(|e| AppError::Internal(e.to_string()))?; }
        let bytes = base64::engine::general_purpose::STANDARD.decode(content_b64).map_err(|_| AppError::ToolError("invalid base64".into()))?;
        fs::write(&full, &bytes).map_err(|e| AppError::Internal(e.to_string()))?;
        if let Some(m) = mode { if let Ok(parsed) = u32::from_str_radix(m, 8) { let perm = fs::Permissions::from_mode(parsed); let _ = fs::set_permissions(&full, perm); } }
